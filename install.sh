...
echo "[3/6] Ubuntu 24.04 설치+dpkg-reconfigure 무력화"

PD_DIR="$PREFIX/var/lib/proot-distro/installed-rootfs"

# (1) 깨끗하지 않은 이전 설치 흔적 제거
for dir in ubuntu ubuntu-24.04; do
  [ -d "$PD_DIR/$dir" ] && [ ! -f "$PD_DIR/$dir/etc/os-release" ] && {
    proot-distro remove "$dir" 2>/dev/null || rm -rf "$PD_DIR/$dir"
  }
done

# (2) 설치 시작 (alias 자동 선택)
alias_name=""
for cand in ubuntu ubuntu-24.04; do
  proot-distro list | awk '{print $1}' | grep -qx "$cand" && { alias_name=$cand; break; }
done

if [ -z "$alias_name" ]; then
  echo "   ▶ rootfs 전개 중 dpkg-reconfigure 무력화 준비"
  proot-distro install ubuntu --override-alias ubuntu-24.04 -- \
    --setup-command 'ln -sf /bin/true /usr/sbin/dpkg-reconfigure' \
  || {
    echo "   ✖ 1차 시도 실패 → 2차 시도"
    proot-distro install ubuntu-24.04 -- \
      --setup-command 'ln -sf /bin/true /usr/sbin/dpkg-reconfigure'
  }
  alias_name=$(proot-distro list | awk '{print $1}' | grep -E '^ubuntu')
else
  echo "   ▶ $alias_name 이미 설치됨"
fi
